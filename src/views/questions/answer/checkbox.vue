<template>
    <div class="value">
        ...
        <el-form-item v-show="index < options.length" v-for="(item,index) in optionsArr" :key="index" :label="'备选项'+(index+1)+'：'">
        ...
        <el-form-item label="正确答案：">
            <el-checkbox-group v-model="answer" >
                <el-checkbox v-for="item in options" :key="item" :label="item" :value="item"></el-checkbox>
            </el-checkbox-group>
        </el-form-item>
    </div>
</template>
...
export default {
    ...
    computed: {
        options () {
            // 直接通过引用传递答案
            this.resultForm.answer = this.answer;
