<template>
    <div class="unpublished">
        <div class="search clearfix section">
            <el-form :inline="true" :model="searchForm" ref="searchForm" class="searchForm">
                ...
                <el-form-item>
                    ...
                    <el-button type="primary" @click="publish">发布考试计划成绩</el-button>
                </el-form-item>
            </el-form>
        </div>
        ...
        <!-- <div class="search clearfix section">
            <el-form :inline="true" :model="searchForm" ref="searchForm" class="searchForm">
                <el-form-item label="请选择需要发布成绩的考试计划：" prop="city">
                    <el-select v-model="searchForm.plan_id" filterable clearable placeholder="请选择" name="city_id">
                        <el-option  v-for="item in plans" :key="item.value" :label="item.plan_name" :value="item.id"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item>
                    <el-button type="primary" @click="publish">一键发布</el-button>
                </el-form-item>
            </el-form>
        </div> -->
        ...
        <div class="exhibit section">
            <el-table ref="multipleTable" :data="initData.lists" border tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
                ...
                <el-table-column prop="practicality_grade" label="实践分数" width="80" align="center">
                </el-table-column>
                <el-table-column prop="human_grade" label="平时分数" width="120" align="center">
                    <template scope="scope">
                        {{scope.row.human_grade ? scope.row.practicality_grade : ''}}
                    </template>
                </el-table-column>
    ...
    export default {
        ...
        methods: {
            ...
            publish () {
                ...
                var _this = this;
                if(!_this.searchForm.plan_id){
                    layer.msg('请先选择要发布的计划', {time: 2500, icon: 2})
                    return;
                }
                layer.confirm('确定要发布该计划吗？', {
                    icon: 7,
                    btn: ['确定', '取消']
                }, function(){
                    scoreApi.scorePublish({
                        plan_id: _this.searchForm.plan_id
                    }).then((res)=>{
                        if(res.data.ret == 0){
                            layer.msg('发布成功', {time: 2500, icon: 1})
                        }else{
                            layer.msg(res.data.res_info, {time: 2500, icon: 1})
                        }
                    })
                });
            },
